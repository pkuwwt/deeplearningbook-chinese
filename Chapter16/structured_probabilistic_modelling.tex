...
\fi
	\caption{自然图片的概率建模。
\emph{(上)}~CIFAR-10数据集\citep{Krizhevsky-Hinton-2009}中的$32\times 32$像素的样例图片。
...
图片经~\citet{Courville-et-al-2011}许可转载。}
...
而另一种常见的语言则是\firstgls{undirected_model}，也被称为\firstall{MRF}或者是\firstgls{markov_network} \citep{Kindermann-1980}。
...
正是基于这个原因，我们把许多\gls{energy_based_model}称为\firstgls{BM}~\citep{Fahlman-et-al-1983,Ackley-et-al-1985,Hinton-et-al-1984,Hinton-Sejnowski-1986}。
...
人们可以将\gls{energy_function}中带有多个项的\gls{energy_based_model}视作是\firstgls{product_of_expert}~\citep{Hinton-1999}。
...
一些\gls{ML}研究者（例如，\citet{Smolensky-1986}将负能量称为~\textbf{harmony}）发出了不同的声音，但这些都不是标准惯例。
% 562  head
...
\citet{Koller-Friedman-2009}是一个不错的\gls{structure_learning}参考资料。
...
\firstall{RBM}\citep{Smolensky-1986}或者\firstgls{harmonium}是\gls{graphical_models}如何用于深度学习的典型例子。 
