...
\begin{itemize}
    ...
    目前，最好的对象识别工作正是基于\gls{DL}的\citep{Krizhevsky-2012,Ioffe+Szegedy-2015}。
...
这类算法超出了本书的范畴。
请参考~\cite{Sutton+Barto-98}或~\cite{Bertsekas+Tsitsiklis-book1996}了解强化学习相关知识，\cite{Mnih-et-al2013}介绍了强化学习方向的\gls{DL}方法。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/30.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/31.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/32.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/33.png}}
\else
...
可惜，即使这样也不能解决整个问题。
\gls{ML}的\firstgls{no_free_lunch_theorem}表明\citep{Wolpert1996}，在所有可能的数据生成分布上平均之后，每一个分类算法在未事先观测的点上都有相同的错误率。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/34.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/35.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/36.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/37.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/38.png}}
\else
\begin{tabular}{ccc}
...
近年来深度学习的很多推动力源自研究局部模版匹配的局限性，以及\gls{DL}如何克服这些局限性\citep{bengio_curse_2006}。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/39.png}}
\else
...
通过这种方式，我们确实能做到非局部的泛化\citep{Bengio+Monperrus-2005,Bengio2006c}。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/40.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/41.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/42.png}}
\else
