...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/156.png}}
\else
...
在这里我们描述 \citet{Neal-Hinton-1999} 所提出的~\glssymbol{EM}~算法。
...
关于$\MH$的最小化问题需要某些特别设计的算法，例如特征符号搜索方法~\citep{Lee-et-al-2007}。
% 629
...
这种完全通用的\gls{graphical_models}方法被称为\firstgls{structured_variational_inference} \citep{Saul-Jordan-1996}。
% 630 
...
为了具体化这些描述，我们接下来会讲如何将变分推断应用到\firstgls{binary_sparse_coding}模型（这里我们所描述的模型是 \citet{Henniges-et-al-2010} 提出的，但是我们采用了传统、通用的\gls{mean_field}方法，而原文作者采用了一种特殊设计的算法）中。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/157.png}}
\else
...
关于在\gls{message_passing}算法中如何选择同步程度以及使用\gls{damping}策略可以参考 \citet{Koller-Friedman-2009} 。
 % 637 head 
...
\gls{DL}中关于变分学习中连续型变量的实际应用可以参考~\citet{Goodfellow-et-al-2013d}。
% 641
...
\firstgls{wake_sleep}算法~\citep{Hinton-et-al-1995b,Frey-et-al-1996}通过从模型分布中抽取$\Vv$和$\Vh$的样本来解决这个问题。
...
然而，该问题可以通过训练深度\gls{encoder}实现\gls{learned}\gls{approximate_inference}来补救，如ISTA技术~\citep{Gregor-LeCun-2010b}。
% 644


近来\gls{learned}\gls{approximate_inference}已经成为了\gls{VAE}形式的\gls{generative_model}中的主要方法之一 \citep{Kingma-2013,Rezende-et-al-2014}。
