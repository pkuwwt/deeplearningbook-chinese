...
最后，我们回过头探讨\gls{representation_learning}成功的原因，从\gls{distributed_representation}~\citep{Hinton-et-al-1986}和深度表示的理论优势，最后会讲到\gls{DGP}潜在假设的更一般概念，特别是观测数据的基本成因。
...
原则上，最后一层可以是另一种模型，如\gls{nearest_neighbor}分类器~\citep{Salakhutdinov-Hinton-2007a}。
...
这种方法至少可以追溯神经认知机~\citep{Fukushima-1975}。
深度学习的复兴始于2006年，源于发现这种贪心学习过程能够为多层联合训练过程找到一个好的初始值，甚至可以成功训练全连接的结构~\citep{Hinton-et-al-2006,Hinton-Salakhutdinov-2006,Hinton-2006,Bengio-et-al-2007,Ranzato-et-al-2007a}。
...
\gls{greedy_layer_wise_unsupervised_pretraining}也能用作其他\gls{unsupervised_learning}算法的初始化，比如深度\gls{AE}~\citep{Hinton-Salakhutdinov-2006}和具有很多\gls{latent_variable}层的概率模型。
这些\gls{model}包括\gls{DBN}~\citep{Hinton-et-al-2006}和\gls{DBM}~\citep{Salakhutdinov-Hinton-2009a}。
...
这是建立在训练浅层模型比\gls{deep_model}更容易的前提下，而该前提似乎在一些情况下已被证实~\citep{Erhan-et-al-2010}。
...
这一观察结果始于2006年对\gls{DNN}的重新关注~\citep{Hinton-et-al-2006,Bengio-et-al-2007,Ranzato-et-al-2007a}。
...
这种单阶段方法的例子包括\gls{discriminative_RBM}~\citep{Larochelle-Bengio-2008}和梯形网络~\citep{Rasmus-et-al-2015}，其中整体目标是两项之和（一个使用\gls{label}，另一个仅仅使用输入）。
...
在2011年，\gls{unsupervised_pretraining}赢得了两个国际\gls{transfer_learning}比赛~\citep{Mesnil-et-al-2011,Goodfellow-et-al-2011}。
在该情景中，目标任务中\gls{labeled}样本的数目很少（每类几个到几十个）。
这些效果也出现在被~\citet{Paine-et-al-2014}严格控制的实验中。
...
例如，它可以提高\gls{DAE}的训练和测试\gls{reconstruction_error}~\citep{Hinton-Salakhutdinov-2006}。

% 524 head

\cite{Erhan-et-al-2010}进行了许多实验来解释\gls{unsupervised_pretraining}的几个成功原因。
...
\cite{Erhan-et-al-2010}也回答了\emph{何时}\gls{pretraining}效果最好——预训练的网络越深，\gls{test_error}的均值和方差下降得越多。
...
经\citet{Erhan-et-al-2010}许可改编此图。
函数空间中的坐标是关于每组输入$\Vx$和它的一个输出$\Vy$的\gls{infinite}维向量。
\cite{Erhan-et-al-2010}将很多特定$\Vx$的$\Vy$连接起来，线性投影到高维空间中。
然后他们使用~\gls{isomap}~\citep{Tenenbaum-et-al-2000}进行进一步的非线性投影并投到二维空间。
颜色表示时间。
...
最通用的方法是在\gls{supervised}阶段使用\gls{validation_set}上的误差来挑选\gls{pretraining}阶段的\gls{hyperparameter}，如~\cite{Larochelle-et-al-2009}中讨论的。
...
这种方法由~\cite{Collobert-Weston-2008b}、\cite{Turian-et-al-2010}和~\cite{Collobert-et-al-2011a}开创，至今仍在使用。
...
在极小的数据集，例如\gls{alternative_splicing_dataset}，贝叶斯方法要优于基于\gls{unsupervised_pretraining}的方法~\citep{Srivastava-2013}。
...
\gls{transfer_learning}中的\gls{supervised_pretraining}流行~\citep{Oquab-et-al-2014,Yosinski-et-al-2014}于在~\ENNAME{ImageNet}~数据集上使用\gls{convolutional_network}\gls{pretraining}。
由于这个原因，实践者们公布了这些网络训练出的参数，就像自然语言任务公布\gls{pretraining}的单词向量一样~\citep{Collobert-et-al-2011a,Mikolov-et-al-2013a}。
...
简单的\gls{unsupervised_pretraining}（\gls{DAE}）已经能够非常成功地用于\gls{domain_adaption}的情感分析\citep{Glorot-et-al-2011b}。
% 527 end 
...
如前所述，\gls{transfer_learning}中\gls{unsupervised}\gls{DL}已经在一些\gls{ML}比赛中取得了成功~\citep{Mesnil-et-al-2011,Goodfellow-et-al-2011}。
这些比赛中的某一个实验配置如下。
...
因为第一阶段学习出的表示就可以清楚地分离出潜在的类别，所以\gls{one_shot_learning}~\citep{Fei-Fei-et-al-2006}是可能的。
...
只有在训练时使用了额外信息，\gls{zero_data_learning}~\citep{Larochelle-et-al-2008}和\gls{zero_shot_learning}~\citep{Palatucci-et-al-2009,Socher-et-al-2013b}才是有可能的。
...
通过使用每个类别词的\gls{word_embeddings}表示，\cite{Socher-et-al-2013b}提出了对象类别的\gls{distributed_representation}。

% 530 mid

我们还可以在\gls{machine_translation}中发现一种类似的现象~\citep{Klementiev-et-al-2012,Mikolov-et-al-2013b,Gouws-et-al-2014}：我们已经知道一种语言中的单词，还可以学到单一语言语料库中词与词之间的关系；另一方面，我们已经翻译了一种语言中的单词与另一种语言中的单词相关的句子。
...
同样的原理可以解释如何能执行\firstgls{multimodal_learning}，学习两种\gls{modality}的表示，和一种\gls{modality}中的观察结果$\Vx$与另一种\gls{modality}中的观察结果$\Vy$组成的对$(\Vx, \Vy)$之间的关系（通常是一个联合分布）\citep{Srivastava-Salakhutdinov-2012}。
...
从20世纪90年代以来，这个想法已经指导了大量的\gls{DL}研究工作~\citep{Becker-Hinton-1992,Hinton-Sejnowski-1999}。
关于\gls{semi_supervised_learning}可以超过纯\gls{supervised_learning}的其他论点，请读者参考\cite{Chapelle-et-al-2006}的第1.2节。
...
根据一个有据可查的心理学现象，人们不会察觉到环境中和他们所在进行的任务并不立刻相关的变化，具体例子可以参考~\cite{Simons-Levin-1998}。
...
实现这样一种定义显著的方法是使用最近提出的\firstgls{generative_adversarial_networks}\citep{Goodfellow-et-al-2014c}。
...
\cite{Lotter-et-al-2015}表明，生成人类头部头像的模型在使用\gls{mean_squared_error}训练时往往会忽视耳朵，但是\gls{adversarial}式框架学习能够成功地生成耳朵。
...
以上图片由~\cite{Lotter-et-al-2015}提供。
}
...
正如~\cite{Scholkopf-et-al-2012}指出，学习\gls{underlying}因素的好处是，如果真实的生成过程中$\RVx$是结果，$\RVy$是原因，那么建模$p(\RVx \mid \RVy)$对于$p(\RVy)$的变化是鲁棒的。
...
甚至深度线性阀值网络的~\glssymbol{VC}~维只有$O(w\log w)$（其中$w$是权重数目）\citep{Sontag-1998}。
...
通过应用关于超平面交集的一般结果~\citep{Zaslavsky-1975}，我们发现~\citep{Pascanu-et-al-2014b}这个二元特征表示能够区分的空间数量是
\begin{equation}
...
例如，\gls{linear_threshold_units}\gls{NN}的~\glssymbol{VC}~维仅为$O(w\log w)$， 其中$w$是权重的数目~\citep{Sontag-1998}。
...
\cite{Zhou-et-al-2015}发现，在~\ENNAME{ImageNet}~和~\ENNAME{Places}~基准数据集上训练的深度\gls{convolutional_network}中的\gls{hidden_unit}\gls{learned}的特征通常是可以解释的，对应人类自然分配的\gls{label}。
...
\cite{Radford-et-al-2015}发现\gls{generative_model}可以学习人脸图像的表示，在表示空间中的不同方向捕获不同的\gls{underlying}\gls{factors_of_variation}。
...
图片转载许可自~\cite{Radford-et-al-2015}。
}
...
许多具有单个\gls{hidden_layer}（\gls{latent_variable}）的\gls{structured_probabilistic_models}（包括\gls{RBM}，\gls{DBN}）是概率分布的\gls{universal_approximator}~\citep{Le-Roux-Bengio-2008,Le-Roux-Bengio-2010,Montufar-Ay-2011,Montufar-2014,Krause-et-al-2013}。
...
\cite{Delalleau-Bengio-2011}表明存在一种\gls{PD}，对~\glssymbol{sum_product_network}~的最小深度有要求，以避免模型规模呈指数级增长。
后来，\cite{Martens-Medabalimi-2014}表明，任意两个有限深度的~\glssymbol{sum_product_network}~之间都会存在显著差异，并且一些使~\glssymbol{sum_product_network}~易于处理的约束可能会限制其表示能力。

% 544 mid

另一个有趣的进展是，一系列和\gls{convolutional_net}相关的\gls{deep_circuit}族表达能力的理论结果，即使让\gls{shadow_circuit}只去近似\gls{deep_circuit}计算的函数，也能突出反映\gls{deep_circuit}的指数级优势~\citep{Cohen-et-al-2015}。
...
该列表在~\cite{Bengio-et-al-2013d}的第3.1节中提出，这里进行了部分拓展。
\begin{itemize}
	...
	参看~\cite{Goodfellow-et-al-2014b}了解关于线性假设局限性的进一步讨论。
...
% -- 546 mid
	...
	很多\gls{ML}算法只在这些\gls{manifold}上有效\citep{Goodfellow-et-al-2014b}。
