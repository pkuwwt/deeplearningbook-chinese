...
数十年间，\gls{AE}的想法一直是\gls{NN}历史景象的一部分~\citep{LeCun-1987,Bourlard-Kamp-1988,Hinton-Zemel-1994}。
...
不同于一般的\gls{feedforward_network}，\gls{AE}也可以使用\firstgls{recirculation}训练\citep{Hinton-McClelland-1988}，这种学习算法基于比较原始输入的激活和\gls{reconstruction}输入的激活。
...
强调与\gls{AE}联系的两个生成式建模方法是~\gls{helmholtz_machine}~\citep{Hinton-et-al-1995b}的衍生模型，如\gls{VAE}（\secref{sec:variational_autoencoders}）和\gls{GSN}（\secref{sec:generative_stochastic_networks}）。
...
\gls{sparse}\gls{AE}的早期工作~\citep{Ranzato-et-al-2007a,Ranzato-et-al-2008}探讨了各种形式的\gls{sparse}性，并提出了\gls{sparse}惩罚和$\log  Z$项（将最大似然应用到无向概率模型$p(\Vx)=\frac{1}{Z}\tilde{p}(\Vx)$时产生）之间的联系。
...
\citet{Glorot-et-al-2011b}提出了一种在\gls{sparse}（和\gls{denoising}）\gls{AE}的$\Vh$中实现\emph{真正为零}的方式。
...
\citet{Alain-Bengio-2013}和~\citet{Bengio-et-al-2013c}指出\gls{denoising}训练过程强制$f$和$g$隐式地学习$p_{\text{data}} (\Vx)$的结构。
...
实验中，深度\gls{AE}能比相应的浅层或线性\gls{AE}产生更好的压缩效率\citep{Hinton-Salakhutdinov-2006}。
...
\citet{Alain-et-al-2015}指出，在保证足够的\gls{capacity}和样本的情况下，将\gls{encoder}和\gls{decoder}作为\gls{DAE}训练，能使它们渐近地相容。
...
对一类采用高斯噪声和\gls{mean_squared_error}作为\gls{reconstruction_error}的特定\gls{DAE}（具有sigmoid~\gls{hidden_unit}和线性\gls{reconstruction}单元）的\gls{denoising}训练过程，与训练一类特定的被称为~\glssymbol{RBM}~的无向概率模型是等价的\citep{Vincent-2011}。
这类模型将在\secref{sec:gaussian_bernoulli_rbms}给出更详细的介绍；对于现在的讨论，我们只需知道这个模型能显式的给出$p_{\text{model}}(\Vx; \Vtheta)$。
当~\glssymbol{RBM}~使用\firstgls{denoising_score_matching}算法~\citep{Kingma-LeCun-2010}训练时，它的学习算法与训练对应的\gls{DAE}是等价的。
...
在~\glssymbol{RBM}~上应用\gls{score_matching}后，其\gls{cost_function}将等价于\gls{reconstruction_error}结合类似~\glssymbol{CAE}~惩罚的正则项 \citep{Swersky-et-al-2011}。
\citet{Bengio-Delalleau-2009}指出\gls{AE}的\gls{gradient}是对~\glssymbol{RBM}~\gls{contrastive_divergence}训练的近似。


对于连续的$\Vx$，高斯损坏和\gls{reconstruction}分布的\gls{denoising}\gls{criterion}得到的\gls{score}估计适用于一般\gls{encoder}和\gls{decoder}的参数化\citep{Alain-Bengio-2013}。
...
经~\citet{Alain-Bengio-2013}许可转载此图。
}
\label{fig:chap14_vector_field_color}
\end{figure}

一般情况下，不能保证\gls{reconstruction}函数$g(f(\Vx))$减去输入$\Vx$后对应于某个函数的\gls{gradient}，更不用说\gls{score} 。
这是早期工作~\citep{Vincent-2011}专用于特定参数化的原因（其中$g(f(\Vx)) - \Vx$能通过另一个函数的导数获得）。
\citet{Kamyshanska-Memisevic-2015}通过标识一类特殊的浅层\gls{AE}家族，使$g(f(\Vx)) - \Vx$对应于这个家族所有成员的一个\gls{score}，以此推广~\citet{Vincent-2011}的结果。
...
采用~\glssymbol{MLP}~\gls{denoising}的想法可以追溯到~\cite{LeCun-1987}和~\citet{Gallinari-et-al-1987}的工作。
...
这个想法提出较晚\citep{Vincent-et-al-2008,Vincent-et-al-2010}。
...
在引入现代~\glssymbol{DAE}~之前，\citet{Inayoshi-Kurita-2005}探索了其中一些相同的方法和目标。
...
如\figref{fig:chap14_faces_graph_manifold}所示，这些方法\citep{Scholkopf-et-al-1998,Roweis-Saul-2000,Tenenbaum-et-al-2000,Brand-2003,Belkin-Niyogi-2003,Donoho-Grimes-2003,Weinberger-Saul-2004,Hinton-Roweis-2003,van-der-Maaten-Hinton-2008}将每个节点与张成实例和近邻之间的差向量变化方向的\gls{tangent_plane}相关联。
...
这些高斯的混合提供了估计的密度函数，如\gls{manifold}中的~\ENNAME{Parzen}~窗口算法\citep{Vincent-Bengio-2003} 或其非局部的基于\gls{NN}的变体\citep{Bengio-et-al-2006c}。
}
...
\gls{CAE}~\citep{Rifai-et-al-2011a,Rifai-et-al-2011b}在\gls{code} $\Vh = f(\Vx)$的基础上添加了显式的正则项，鼓励$f$的导数尽可能小：
\begin{align}
...
\gls{DAE}和\gls{CAE}之间存在一定联系：\citet{Alain-Bengio-2013}指出在小高斯噪声的限制下，当\gls{reconstruction}函数将$\Vx$映射到$\Vr = g(f(\Vx))$时，\gls{denoising}\gls{reconstruction_error}与\gls{contractive}惩罚项是等价的。
...
\citet{Rifai-et-al-2011a,Rifai-et-al-2011b}的实验显示训练~\glssymbol{CAE}~会导致$\MJ$中大部分奇异值（幅值）比1小，因此是收缩的。
...
经~\citet{Rifai-et-al-2011c}许可转载此图。
}
...
根据~\citet{Rifai-et-al-2011a}的策略，分别训练一系列单层的\gls{AE}，并且每个被训练为\gls{reconstruction}前一个\gls{AE}的\gls{hidden_layer}。
...
\citet{Rifai-et-al-2011a}通过绑定$f$和$g$的权重来防止这种情况。
...
\firstall{PSD}是\gls{sparse_coding}和参数化\gls{AE}~\citep{Kavukcuoglu-et-al-2008}的混合模型。
参数化\gls{encoder}被训练为能预测迭代推断的输出。
\glssymbol{PSD}~被应用于图片和视频中对象识别的\gls{unsupervised}特征学习\citep{Kavukcuoglu-et-al-2009,Kavukcuoglu-et-al-2010,Jarrett-et-al-2009,Farabet-et-al-2011}，在音频中也有所应用\citep{Henaff-et-al-2011}。
...
它是研究\gls{AE}早期驱动力之一。
例如， \citet{Hinton-Salakhutdinov-2006}训练了一个栈式~\glssymbol{RBM}，然后利用它们的权重初始化一个\gls{hidden_layer}逐渐减小的深度\gls{AE}，终结于30个单元的瓶颈。
...
小空间的模型消耗更少的内存和运行时间。
据~\citet{Salakhutdinov-Hinton-2007b}和~\citet{Torralba-et-al-2008}观察，许多\gls{dimensionality_reduction}的形式会将语义上相关的样本置于彼此邻近的位置。
映射到低维空间所提供的线索有助于泛化。
...
这种通过\gls{dimensionality_reduction}和二值化的\gls{information_retrieval}方法被称为\firstgls{semantic_hashing}\citep{Salakhutdinov-Hinton-2007b,Salakhutdinov-Hinton-2009b}，已经被用于文本输入\citep{Salakhutdinov-Hinton-2007b,Salakhutdinov-Hinton-2009b}和图像\citep{Torralba-et-al-2008,Weiss-et-al-2008,Krizhevsky-Hinton-2011}。
...
学习哈希函数的思想已在其他多个方向进一步探讨，包括改变损失训练\gls{representation}的想法，其中所需优化的损失与哈希表中查找附近样本的任务有更直接的联系\citep{Norouzi-Fleet-2011}。
