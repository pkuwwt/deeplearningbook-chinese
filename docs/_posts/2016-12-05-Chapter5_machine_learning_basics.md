...
我们建议新手读者或是希望更全面了解的读者参考一些更全面覆盖基础知识的机器学习参考书，例如~{Murphy-2012}或者~{Bishop-2006}。
...
+ \textbf{输入缺失分类}：
    ...
    参见~{Goodfellow-et-al-2013b}了解以这种方式将深度概率模型应用于这类任务的示例。 
...
+ \textbf{结构化输出}：结构化输出任务的输出是向量或者其他包含多个值的数据结构，并且构成输出的这些不同元素间具有重要关系。
    ...
    参考~{Collobert-2011}将深度学习应用到语法分析的示例。
...
+ \textbf{异常检测}：在这类任务中，计算机程序在一组事件或对象中筛选，并标记不正常或非典型的个体。
    ...
    参考~{Chandola-et-al-2009}了解欺诈检测方法。
...
参考~{Kotzias-et-al-2015}了解最近深度模型进行多实例学习的示例。

有些机器学习算法并不是训练于一个固定的数据集上。
例如，强化学习算法会和环境进行交互，所以学习系统和它的训练过程会有反馈回路。
这类算法超出了本书的范畴。
请参考~{Sutton-Barto-1998}或~{Bertsekas-Tsitsiklis-1996}了解强化学习相关知识，{Mnih-et-al-2013}介绍了强化学习方向的深度学习方法。
...
当前深度学习的复兴始于~{Hinton-et-al-2006}表明神经网络能够在MNIST基准数据上胜过RBF核的支持向量机。
