...
在这里我们描述 {Neal-Hinton-1999} 所提出的~EM~算法。
...
为了具体化这些描述，我们接下来会讲如何将变分推断应用到二值稀疏编码模型（这里我们所描述的模型是 {Henniges-et-al-2010} 提出的，但是我们采用了传统、通用的均值场方法，而原文作者采用了一种特殊设计的算法）中。
...
关于在信息传输算法中如何选择同步程度以及使用衰减策略可以参考 {Koller-Friedman-2009} 。
 % 637 head 
...
深度学习中关于变分学习中连续型变量的实际应用可以参考~{Goodfellow-et-al-2013d}。
<!-- % 641 -->
