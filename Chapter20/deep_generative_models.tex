...
\gls{BM}最初作为一种广义的``\gls{connectionism}''引入，用来学习二值向量上的任意概率分布~\citep{Fahlman-et-al-1983,Ackley-et-al-1985,Hinton-et-al-1984,Hinton-Sejnowski-1986}。
...
相反，\gls{BM}变成了离散变量上\gls{PMF}的\gls{universal_approximator} \citep{Le-Roux-Bengio-2008}。
...
这是~\ENNAME{Hebbian}~学习规则\citep{Hebb-1949}的一个例子 ，经常总结为好记的短语——``fire together, wire together''。
\ENNAME{Hebbian}~学习规则是生物系统学习中最古老的假设性解释之一，直至今天仍然有重大意义 \citep{Giudice-et-al-2009}。


不仅仅使用局部统计信息的其他学习算法似乎需要假设更多的学习机制。
例如，对于大脑在\gls{MLP}中实现的\gls{back_propagation}，似乎需要维持一个辅助通信的网络，并借此向后传输梯度信息。
已经有学者\citep{Hinton-2007a,Bengio-2015} 提出生物学上可行（和近似）的\gls{back_propagation}实现方案，但仍然有待验证，\citet{Bengio-2015} 还将梯度的\gls{back_propagation}关联到类似于\gls{BM}（但具有连续\gls{latent_variable}）的能量模型中的\gls{inference}。
...
\gls{RBM}以\firstgls{harmonium}之名\citep{Smolensky-1986}面世之后，成为了深度概率模型中最常见的组件之一。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/158.png}}
\else
\centering
\begin{tabular}{cc}
...
在\gls{RBM}的情况下， \citet{Long-Servedio-2010}正式证明\gls{partition_function} $Z$是难解的。
...
\firstall{DBN}是第一批成功应用深度架构训练的非卷积模型之一\citep{Hinton-et-al-2006,Hinton-2007b}。
2006年\gls{DBN}的引入开始了当前深度学习的复兴。
在引入\gls{DBN}之前，深度模型被认为太难以优化。
具有凸\gls{objective_function}的\gls{kernel_machines}引领了研究前沿。
\gls{DBN}在MNIST数据集上表现超过内核化支持向量机，以此证明深度架构是能够成功的\citep{Hinton-et-al-2006}。
...
这个过程可以被视为提高数据在~\glssymbol{DBN}~下似然概率的变分下界\citep{Hinton-et-al-2006}。
...
\firstall{DBM} \citep{Salakhutdinov-Hinton-2009a}是另一种深度\gls{generative_model}。
...
\gls{DBM}已经被应用于各种任务，包括文档建模\citep{Srivastava-et-al-2013}。

\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/159.png}}
\else
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/160.png}}
\else
...
由于这个性质，\glssymbol{DBM}~已被用作真实神经科学现象的计算模型 \citep{Series-et-al-2010,Reichert-et-al-2011}。
...
在本节中，我们推导出由~\cite{Salakhutdinov-Hinton-2009a}最初引入的迭代近似\gls{inference}过程。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/161.png}}
\else
\centerline{\includegraphics{Chapter20/figures/standard_dbm_color}}
\fi
\caption{用于分类MNIST数据集的\gls{DBM}训练过程~\citep{Salakhutdinov-Hinton-2009a,Srivastava-et-al-2014}。
...
图来自~\citet{Goodfellow-et-al-2013b}。
}
...
为了解释这种效应，\citet{Salakhutdinov-Hinton-2009a}提倡在将其插入~\glssymbol{DBM}~之前，将所有~\glssymbol{RBM}（顶部和底部~\glssymbol{RBM}~除外）的权重除2。
另外，必须使用每个可见单元的两个``副本''来训练底部~\glssymbol{RBM}，并且两个副本之间的权重约束为相等。
这意味着在向上传播时，权重能有效地加倍。
类似地，顶部~\glssymbol{RBM}~应当使用最顶层的两个副本来训练。

为了使用\gls{DBM}获得最好结果，我们需要修改标准的~\glssymbol{SML}~算法，即在联合~\glssymbol{persistent_contrastive_divergence}~训练步骤的\gls{negative_phase}期间使用少量的\gls{meanfield} \citep{Salakhutdinov-Hinton-2009a}。
具体来说，应当相对于其中所有单元彼此独立的\gls{meanfield}分布来计算能量梯度的期望。
这个\gls{meanfield}分布的参数应该通过运行一次\gls{meanfield}\gls{fixed_point_equation}获得。
\citet{Goodfellow-et-al-2013b}比较了在\gls{negative_phase}中使用和不使用部分\gls{meanfield}的中心化~\glssymbol{DBM}~的性能。
...
第一个是\textbf{中心化\gls{DBM}}(centered deep Boltzmann machine) \citep{Montavon-Muller-2012}，通过\gls{reparametrization}模型使其在开始学习过程时\gls{cost_function}的~\gls{hessian}~具有更好的条件数。
...
联合训练\gls{DBM}的第二种方式是使用\firstall{MPDBM} \citep{Goodfellow-et-al-2013b}。
...
\citet{Melchior-et-al-2013}通过实验证实了~\gls{hessian}~矩阵条件数的改善，并观察到中心化技巧等价于另一个\gls{BM}学习技术——\textbf{增强梯度}(enhanced gradient) \citep{Cho-et-al-2011}。
即使在困难的情况下，例如训练多层的\gls{DBM}，\gls{hessian}~矩阵条件数的改善也能使学习成功。

% -- 664 --

联合训练\gls{DBM}的另一种方法是\glsacr{MPDBM}，它将\gls{meanfield}方程视为定义一系列用于近似求解每个可能\gls{inference}问题的\gls{recurrent_network}~\citep{Goodfellow-et-al-2013b}。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/162.png}}
\else
...
图改编自~\citet{Goodfellow-et-al-2013b}。
}
\label{fig:chap20_multi_prediction_training}
\end{figure}

这种用于近似\gls{inference}，通过\gls{computational_graph}进行\gls{back_propagation}的一般原理已经应用于其他模型~\citep{Stoyanov-et-al-2011,Brakel-et-al-2013}。
...
例如， \citet{Hinton-2000}将训练集中灰度图像的像素值视为定义$[0,1]$间的概率值。
...
\gls{RBM}可以用于许多指数族的条件分布 \citep{Welling-et-al-2005}。
...
虽然\gls{gaussian_rbm}~已成为实值数据的标准能量模型， \citet{Ranzato-et-al-2010a}认为\gls{gaussian_rbm}~感应\gls{bias_aff}不能很好地适合某些类型的实值数据中存在的统计变化，特别是自然图像。
...
另一半是\firstall{crbm} \citep{Ranzato-et-al-2010a}，对条件协方差的结构进行建模（如下所述）。
...
\citet{Ranzato-Hinton-2010}通过使用~\glssymbol{mcrbm}~自由能上的哈密尔顿（混合）\gls{monte_carlo}~\citep{Neal-1993}直接从边缘$p(\Vx)$采样，避免了直接从条件$  p_{\text{mc}}(\Vx  \mid \Vh^{(m)}, \Vh^{(c)}) $抽样。

 % -- 670 --
 
\paragraph{\gls{mpot}}
\glsacr{mpot}模型~\citep{Ranzato-et-al-2010b}以类似~\glssymbol{mcrbm}~扩展~\glssymbol{crbm}~的方式扩展PoT模型~\citep{Welling-et-al-2003a}。
...
因此~\citet{Ranzato-et-al-2010b} 也倡导通过哈密尔顿（混合）\gls{monte_carlo}~\citep{Neal-1993}直接采样$p(\Vx)$。


\paragraph{\gls{ssrbm}} \firstall{ssrbm}\citep{Courville-et-al-2011}提供对实值数据的协方差结构建模的另一种方法。
...
\glssymbol{ssrbm}~条件协方差与一个不同模型给出的类似：概率主成分分析的乘积（PoPPCA）\citep{Williams-Agakov-2002}。
...
包括平板变量的高阶交互和平均池化\citep{Courville-et-al-2014} 使得模型能够在标注数据稀缺时为分类器学习到出色的特征。
向\gls{energy_function}添加一项能防止\gls{partition_function}在\gls{sparse_coding}模型下变得不确定，如尖峰和平板\gls{sparse_coding}~\citep{Goodfellow-et-al-2013d}，也称为S3C。
...
\citet{Desjardins-Bengio-2008} 表明这种方法应用于~\glssymbol{RBM}~时效果很好。
...
~\citet{Lee-et-al-2009} 针对这个问题，开发了一个称为\,\textbf{概率最大池化}(probabilistic max pooling)的解决方案（不要与``随机池化''混淆，``随机池化''是用于隐含地构建卷积前馈网络集成的技术）。
...
\citet{Lee-et-al-2009} 证明概率最大池化可以用于构建卷积\gls{DBM}\footnote{该论文将模型描述为``\gls{DBN}''，但因为它可以被描述为纯无向模型（具有易处理逐层\gls{meanfield}不动点更新），所以它最适合\gls{DBM}的定义。}。
...
对于\gls{BM}，大型池化区域的计算成本比朴素方法高很多。
 \citet{Lee-et-al-2009} 的方法使得每个检测器单元在相同的池化区域中互斥，解决了计算问题，但仍然不允许大小可变的池化区域。
...
为了解决这个序列建模任务，\citet{Taylor-et-al-2007} 针对小的$m$引入了条件~\glssymbol{RBM}~建模$p(\Vx^{(t)}  \mid  \Vx^{(t-1)}, \dots, \Vx^{(t-m)})$。
...
条件~\glssymbol{RBM}~的其他变体 \citep{Mnih-et-al-2011} 和使用条件~\glssymbol{RBM}~进行序列建模的其他变体是可能的 \citep{Taylor-Hinton-2009,Sutskever-et-al-2009,Boulanger-Lewandowski-et-al-2012}。

另一个序列建模任务是对构成歌曲音符序列的分布进行建模。
\citet{Boulanger-Lewandowski-et-al-2012} 引入了\textbf{RNN-RBM}序列模型并应用于这个任务。
...
相反，旨在最大化$\log p(y \mid \Vv)$来训练判别的~\glssymbol{RBM}~也是有可能的\citep{Larochelle-Bengio-2008}。
...
我们还可以训练高阶\gls{BM}~\citep{Sejnowski-1987} ，其中\gls{energy_function}项涉及许多变量的乘积。
\gls{hidden_unit}和两个不同图像之间的三向交互可以建模从一个视频帧到下一个帧的空间变换 \citep{Memisevic-Hinton-2007,Memisevic-Hinton-2010}。
通过\gls{one_hot}类别变量的乘法可以根据存在哪个类来改变可见单元和\gls{hidden_unit}之间的关系\citep{Nair-Hinton-2009}。
使用高阶交互的一个最近的示例是具有两组\gls{hidden_unit}的\gls{BM}，一组同时与可见单元$\Vv$和类别标签$y$交互，另一组仅与输入值$\Vv$交互\citep{Luo-et-al-2011}。 % ?
这可以被解释为鼓励一些\gls{hidden_unit}学习使用与类相关的特征来建模输入，而且还学习额外的\gls{hidden_unit}（不需要根据样本类别，学习逼真$\Vv$样本所需的繁琐细节）。
高阶交互的另一个用途是选通一些特征。
\citet{Sohn-et-al-2013} 介绍了一个带有三阶交互的\gls{BM}，以及与每个可见单元相关的二进制掩码变量。
...
最近，它已被应用于变分近似~\citep{Opper-Archambeau-2009} 和随机生成神经网络~\citep{Bengio-et-al-2013b,Kingma-2013,Kingma-Welling-2014b,Kingma-Welling-2014a,Rezende-et-al-2014,Goodfellow-et-al-2014c}。
...
通过使用\firstgls{variance_reduction}方法~\citep{Wilson-1984,L'Ecuyer-1994}，可以地减少该估计的方差。
...
\citet{Mnih-Gregor-2014} 倾向于使用通过目标$J(\Vy)$训练的单个共享输出（跨越$\Vomega$的所有元素$i$），并使用$b(\Vomega) \approx E_{p(\Vy)} [J(\Vy)]$作为基线。

% -- 681 --

在\gls{RL}背景下引入的\gls{variance_reduction}方法\citep{Sutton-et-al-2000,Weaver-Tao-2001}， \citet{Dayan-1990}推广了二值奖励的前期工作。
可以参考 \citet{Bengio-et-al-2013b}、\citet{Mnih-Gregor-2014}、\citet{Ba-et-al-2014}、\citet{Mnih-et-al-2014}或 \citet{Xu-et-al-2015} 中在深度学习的背景下使用减少方差的~\ENNAME{REINFORCE}~算法的现代例子。
除了使用与输入相关的基线$b(\Vomega)$，\citet{Mnih-Gregor-2014}发现可以在训练期间调整$(J(\Vy) - b(\Vomega))$的尺度（即除以训练期间的移动平均估计的标准差 ），即作为一种适应性\gls{learning_rate}，可以抵消训练过程中该量大小发生的重要变化的影响。
\citet{Mnih-Gregor-2014} 称之为启发式\textbf{方差归一化}(variance normalization)。
...
在~\gls{sigmoid_bn}中执行\gls{inference}的一种方法是构造专用于~\gls{sigmoid_bn}的不同下界 \citep{Saul-et-al-1996}。
这种方法只适用于非常小的网络。
另一种方法是使用\gls{learned}\gls{inference}机制，如\secref{sec:learned_approximate_inference}中描述的。
\gls{helmholtz_machine} \citep{Dayan-et-al-1995,Dayan-Hinton-1996} 结合了一个~\gls{sigmoid_bn}与一个预测\gls{hidden_unit}上\gls{meanfield}分布参数的\gls{inference}网络。
\gls{sigmoid_bn}的现代方法\citep{Gregor-et-al-2014,Mnih-Gregor-2014} 仍然使用这种\gls{inference}网络的方法。
...
最近基于\gls{importance_sampling}、重加权的\gls{wake_sleep}\citep{Bornschein-Bengio-2015} 或双向~\gls{helmholtz_machine}~\citep{Bornschein-et-al-2015} 的方法使得我们可以快速训练~\gls{sigmoid_bn}，并在基准任务上达到最好的表现。
...
例如，\textbf{逆变换采样}(inverse transform sampling)\citep{Devroye-2013}从$U(0,1)$中采一个标量$z$，并且对标量$x$应用非线性变换。 % ??
...
\citet{Dosovitskiy-et-al-2015}研究了一个简化问题，其中$\Vz$和$\Vx$之间的对应关系已经给出。
具体来说，训练数据是计算机渲染的椅子图。
...
\firstall{VAE}\citep{Kingma-2013,Rezende-et-al-2014}是一个使用学好的近似\gls{inference}的\gls{directed_model}， 可以纯粹地使用基于梯度的方法进行训练。
...
如~\citet{Theis-et-al-2015}和~\citet{Huszar-2015}指出的，该问题不是~\glssymbol{VAE}~特有的，而是与优化对数似然或$D_{\text{KL}}(p_{\text{data}} ||p_{\text{model}} )$的\gls{generative_model}共享的。
...
一个特别复杂的~\glssymbol{VAE}~是\textbf{深度循环注意写者}(DRAW)模型\citep{Gregor-et-al-2015}。
DRAW使用一个循环编码器和循环解码器并结合\gls{attention_mechanism}。
DRAW模型的生成过程包括顺序访问不同的小图像块并绘制这些点处的像素值。
我们还可以通过在~\glssymbol{VAE}~框架内使用循环编码器和解码器来定义变分~\glssymbol{RNN}~\citep{Chung-et-al-2015b}来扩展~\glssymbol{VAE}~以生成序列。
从传统~\glssymbol{RNN}~生成样本仅在输出空间涉及非确定性操作。
而变分~\glssymbol{RNN}~还具有由~\glssymbol{VAE}~\gls{latent_variable}捕获的潜在更抽象层的随机变化性。

% -- 688 --

\glssymbol{VAE}~框架已不仅仅扩展到传统的变分下界，还有\,\textbf{重要加权\gls{AE}}(importance-weighted autoencoder)\citep{Burda-et-al-2015}的目标：
\begin{align}
...
\gls{VAE}与~\glssymbol{MPDBM}~和其他涉及通过近似\gls{inference}图的\gls{back_propagation}方法有一些有趣的联系 \citep{Goodfellow-et-al-2013b,Stoyanov-et-al-2011,Brakel-et-al-2013}。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/163.png}}
\else
\centerline{
...
\fi
\caption{由\gls{VAE}学习的高维\gls{manifold}在2维坐标系中的示例\citep{Kingma-Welling-2014a}。
...
\firstall{GAN}\citep{Goodfellow-et-al-2014c}是基于可微\gls{generator_network}的另一种\gls{generative_modeling}方法。
...
\citet{Goodfellow-2014}认为不收敛可能会引起~\glssymbol{GAN}~的\gls{underfitting}问题。
...
\citet{Goodfellow-2014}确定了另一种替代的形式化收益公式，其中博弈不再是零和，每当判别器最优时， 具有与最大似然学习相同的预期梯度。
因为最大似然训练收敛，这种~\glssymbol{GAN}~博弈的重述在给定足够的样本时也应该收敛。
不幸的是，这种替代的形式化似乎并没有提高实践中的收敛，可能是由于判别器的次优性或围绕期望梯度的高方差。

% -- 691 --

在真实实验中，\glssymbol{GAN}~博弈的最佳表现形式既不是零和也不等价于最大似然，而是\citet{Goodfellow-et-al-2014c}引入的带有启发式动机的不同形式化。
...
\citet{Radford-et-al-2015}设计了一个深度卷积~\glssymbol{GAN}（DCGAN），在图像合成的任务上表现非常好，并表明其\gls{latent}的表示空间能捕获到变化的重要因素，如\figref{fig:chap15_generative_glasses}所示。
\figref{fig:chap20_lsun_small}展示了DCGAN生成器生成的图像示例。
 
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/164.png}}
\else
\centering
$\vcenter{\hbox{
...
\emph{(左)}由DCGAN模型生成的卧室图像，经\citet{Radford-et-al-2015}许可转载。
\emph{(右)}由LAPGAN模型生成的教堂图像，经 \citet{Denton-et-al-2015}许可转载。
}
\label{fig:chap20_lsun_small}
\end{figure}

\glssymbol{GAN}~学习问题也可以通过将生成过程分成许多级别的细节来简化。
我们可以训练有条件的~\glssymbol{GAN}~\citep{Mirza-Osindero-2014}，并学习从分布$p(\Vx \mid \Vy)$中采样，而不是简单地从边缘分布$p(\Vx)$中采样。
\citet{Denton-et-al-2015} 表明一系列的条件~\glssymbol{GAN}~可以被训练为首先生成非常低分辨率的图像，然后增量地向图像添加细节。
...
例如，\textbf{自监督提升}( self-supervised boosting)可以用于训练~\glssymbol{RBM}~生成器以欺骗\gls{logistic_regression}判别器~\citep{Welling-et-al-2002}。
...
\firstgls{generative_moment_matching_network}\citep{Li-et-al-2015,Dziugaite-et-al-2015}是另一种基于可微\gls{generator_network}的\gls{generative_model}。
...
相反，我们可以通过最小化一个被称为\firstall{MMD}\citep{Scholkopf-Smola-2002,Gretton-et-al-2012}的\gls{cost_function}来训练\gls{generative_moment_matching_network}。
...
当生成图像时，将卷积结构的引入\gls{generator_network}通常是有用的（见~\citet{Goodfellow-et-al-2014c}或~\citet{Dosovitskiy-et-al-2015}的例子）。
...
更简单的操作是仅仅增加表示的空间大小。
似乎可接受的方法是使用~\citet{Dosovitskiy-et-al-2015}引入的``去池化''。
...
这样的模型被称为\,\textbf{完全可见的贝叶斯网络}（fully-visible Bayes networks, FVBN），并成功地以许多形式使用，首先是对每个条件分布\gls{logistic_regression}~\citep{Frey-1998} ，然后是带有\gls{hidden_unit}的神经网络~\citep{Bengio-Bengio-2000b,Larochelle-Murray-2011}。
在某些形式的\gls{auto_regressive_network}中， 例如在\secref{sec:nade}中描述的~\glssymbol{NADE}~\citep{Larochelle-Murray-2011}，我们可以引入\gls{parameter_sharing}的一种形式，它能带来统计优点（较少的唯一参数）和计算优势 （较少计算量）。
...
这个模型由~\citet{Frey-1998} 引入，当有$d$个变量要建模时，该模型有$\CalO(d^2)$个参数。
如\figref{fig:chap20_fvbn}所示。

\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/165.png}}
\else
...
\gls{neural_auto_regressive_network} \citep{Bengio-Bengio-2000a,Bengio-Bengio-2000b}具有与逻辑\gls{auto_regressive_network}相同的从左到右的\gls{graphical_model}（\figref{fig:chap20_fvbn}），但在该\gls{graphical_model}结构内采用不同的条件分布参数。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/166.png}}
\else
...
\firstall{NADE}是最近非常成功的\gls{neural_auto_regressive_network}的一种形式 \citep{Larochelle-Murray-2011}。
与~\citet{Bengio-Bengio-2000b}的原始\gls{neural_auto_regressive_network}中的连接相同，但~\glssymbol{NADE}~引入了附加的\gls{parameter_sharing}方案，如\figref{fig:chap20_NADE}所示。
...
\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/167.png}}
\else
...
\citet{Larochelle-Murray-2011} 选择了这种共享方案，使得~\glssymbol{NADE}~模型中的正向传播与在\gls{meanfield}\gls{inference}中执行的计算大致相似，以填充~\glssymbol{RBM}~中缺失的输入。
...
一个称为~\ENNAME{RNADE}~的模型\citep{Uria-et-al-2013} 使用这种参数化将~\glssymbol{NADE}~扩展到实值。
...
为了减少这种困难， \citet{Uria-et-al-2013}在后向传播阶段使用伪梯度代替平均值上的梯度。

另一个非常有趣的神经自回归架构的扩展摆脱了为观察到的变量选择任意顺序的需要\citep{Murray-Larochelle-2014}。
...
在同一篇文章中，作者提出了深度版本的架构，但不幸的是，这立即使计算成本像原始\gls{neural_auto_regressive_network}一样高\citep{Bengio-Bengio-2000b}。
第一层和输出层仍然可以在$\CalO(nh)$的乘法-加法操作中计算，如在常规~\glssymbol{NADE}~中，其中$h$是\gls{hidden_unit}的数量（\figref{fig:chap20_NADE}和\figref{fig:chap20_neural_autoregressive}中的组$h_i$的大小），而它在~\citet{Bengio-Bengio-2000b}中是$\CalO(n^2h)$。
然而，对于其他\gls{hidden_layer}的计算量是$\CalO(n^2 h^2)$（假设在每个层存在$n$组$h$个\gls{hidden_unit}，且在$l$层的每个``先前''组参与预测$l+1$层处的``下一个''组）。
如在~\citet{Murray-Larochelle-2014}中， 使$l+1$层上的第$i$个组仅取决于第$i$个组，$l$层处的计算量将减少到$\CalO(nh^2)$，但仍然比常规~\glssymbol{NADE}~差$h$倍。
...
这意味着使用注入噪声的重复编码和解码将引起沿着流形表面的随机游走\citep{Rifai-et-al-2012,Mesnil-et-al-2012}。
...
\citet{Bengio-et-al-2013c}展示了如何构建这种用于\textbf{广义\gls{DAE}}(generalized denoising autoencoder)的\gls{markov_chain}。
...
\end{enumerate}
\citet{Bengio-et-al-2014} 表明，如果\gls{AE} $p(\RVx  \mid  \tilde{\Vx})$形成对应真实条件分布的一致估计量，则上述\gls{markov_chain}的平稳分布形成数据生成分布$\RVx$的一致估计量（虽然是隐式的）。

\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/168.png}}
\else
...
后者的噪声水平应对应于\gls{reconstruction}的\gls{mean_squared_error}，而注入的噪声是控制混合速度以及估计器平滑经验分布程度的超参数\citep{Vincent-2011}。
在这所示的例子中，只有$C$和$p$条件是随机步骤（$f$和$g$是确定性计算），我们也可以在\gls{AE}内部注入噪声，如\gls{GSN}~\citep{Bengio-et-al-2014}。
}
...
\glssymbol{GSN}~随后将~\glssymbol{MPDBM}~中的一些想法推广以执行相同的操作~\citep{Bengio-et-al-2014}。
\citet{Alain-et-al-2015} 从~\citet{Bengio-et-al-2014}的命题1中发现了一个缺失条件，即转移算子（由从链的一个状态到下一个状态的随机映射定义）应该满足\firstgls{detailed_balance}的属性， 表明无论转移算子正向或反向运行，\gls{markov_chain}都将保持平衡。

在\figref{fig:chap20_inpainting1500-half_croppedhalf__with_nearest}中展示了夹合一半像素（图像的右部分）并在另一半上运行\gls{markov_chain}的实验。

\begin{figure}[!htb]
\ifOpenSource
\centerline{\includegraphics[scale=0.5]{images/169.png}}
\else
...
回退训练过程由~\citet{Bengio-et-al-2013c} 等人提出，作为一种加速\gls{DAE}生成训练收敛的方法。
...
\firstall{GSN} \citep{Bengio-et-al-2014} 是\gls{DAE}的推广，除可见变量（通常表示为$\RVx$）之外，在生成\gls{markov_chain}中还包括\gls{latent_variable} $\RVh$。
...
由~\citet{Bengio-et-al-2014} 提出和评估的只对可见单元上对数概率的重建，如应用于\gls{DAE}。
通过将$\RVx^{(0)} = \Vx$ 夹合到观察到的样本并且在一些后续\gls{time_step}处使生成$\Vx$的概率最大化，即最大化$\log p(\RVx^{(k)} = \Vx  \mid  \RVh^{(k)})$， 其中给定$\RVx^{(0)} = \Vx$后，$\RVh^{(k)}$从链中采样。
为了估计相对于模型其他部分的$\log p(\RVx^{(k)} = \Vx  \mid  \RVh^{(k)})$的梯度，\citet{Bengio-et-al-2014}使用了在\secref{sec:back_propagation_through_random_operations}中介绍的\gls{reparametrization_trick}。

回退训练过程（在\secref{sec:walk_back_training_procedure}中描述）可以用来改善训练~\glssymbol{GSN}~的收敛性~\citep{Bengio-et-al-2014} 。


\subsection{判别性\,\glssymbol{GSN}}
\label{sec:discriminant_gsns}
\glssymbol{GSN}的原始公式\citep{Bengio-et-al-2014} 用于\gls{unsupervised_learning}和对观察数据$\RVx$的$p(\RVx)$的隐式建模，但是我们可以修改框架来优化$p(\RVy  \mid  \Vx)$。


例如， \citet{Zhou-Troyanskaya-2014} 以如下方式推广~\glssymbol{GSN}，只\gls{back_propagation}输出变量上的重建对数概率，并保持输入变量固定。
...
\citet{Zohrer-Pernkopf-2014} 引入了一个混合模型，通过简单地添加（使用不同的权重）监督和非监督成本即$\RVy$和$\RVx$的重建对数概率， 组合了监督目标（如上面的工作）和无监督目标（如原始的~\glssymbol{GSN}）。
\citet{Larochelle-Bengio-2008}以前在~\glssymbol{RBM}~中就提出了这样的混合标准。
他们展示了在这种方案下分类性能的提升。
...
\citet{Sohl-Dickstein-et-al-2015} 开发了一种基于非平衡热力学学习\gls{generative_model}的\,\textbf{扩散反演}(diffusion inversion)训练方案。
...
在这个意义上，没有由迭代过程诱导的近似。
\citet{Sohl-Dickstein-et-al-2015} 介绍的方法也非常接近于\gls{DAE}的生成解释（\secref{sec:markov_chain_associated_with_any_denoising_autoencoder}）。
...
样本生成的另一种方法是\firstall{ABC}框架\citep{Rubin-1984}。
...
\citet{Bachman-Precup-2015} 展示了如何在深度学习的背景下使用~\glssymbol{ABC}~中的想法，即使用~\glssymbol{ABC}~来\gls{shaping}~\glssymbol{GSN}~的~\glssymbol{mcmc}~轨迹。
...
在最好的情况下，这不是由研究人员本身，而是由不知道样品来源的实验受试者完成 \citep{Denton-et-al-2015}。
...
\citet{Theis-et-al-2015} 回顾了评估\gls{generative_model}所涉及的许多问题，包括上述的许多想法。
