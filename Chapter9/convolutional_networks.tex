...
\firstgls{convolutional_network}\citep{LeCun-1989}，也叫做\firstall{CNN}，是一种专门用来处理具有类似网格结构的数据的神经网络。
...
例如，\firstgls{max_pooling}函数\citep{Zhou-Chellappa-1988}给出相邻矩形区域内的最大值。
...
这个原则在\,\gls{maxout}\,网络\citep{Goodfellow-et-al-2013a}和其他卷积网络中更有影响。
...
一些理论工作对于在不同情况下应当使用哪种\gls{pooling}函数给出了一些指导\citep{Boureau-et-al-2010}。
将特征一起动态地\gls{pooling}也是可行的，例如，对于感兴趣特征的位置运行聚类算法\citep{Boureau-et-al-2011}。
这种方法对于每幅图像产生一个不同的\gls{pooling}区域集合。
另一种方法是先\emph{学习}一个单独的\gls{pooling}结构，再应用到全部的图像中\citep{Jia-et-al-2012}。
...
一些\gls{convolutional_network}结构~\citep{Szegedy-et-al-2014a}为了既获得具有较高不变性的特征又获得当平移不变性不合理时不会导致\gls{underfitting}的特征，被设计成在一些通道上使用\gls{pooling}而在另一些通道上不使用。
...
在一些情况下，我们并不是真的想使用卷积，而是想用一些局部连接的网络层\citep{LeCun-1986,LeCun-1989}。
...
\firstgls{tiled_convolution}\citep{Gregor-LeCun-2010a,Le-et-al-2010}对卷积层和局部连接层进行了折衷。
...
如果我们想要从\gls{hidden_layer}单元重构可视化单元时，同样的运算也是需要的\citep{Simard-et-al-1992}。
...
对于完全一般的多维、多样例情况下的公式，完整的推导可以参考~\cite{Goodfellow-2010}。 
...
为了产生与输入大小相似的输出映射，我们可以避免把\gls{pooling}放在一起\citep{Jain-et-al-2007}。
另一种策略是单纯地产生一张低分辨率的标签网格\citep{Pinheiro-Collobert-2014,Pinheiro-Collobert-2015}。
最后，原则上可以使用具有单位\gls{stride}的\gls{pooling}操作。

对图像逐个像素标记的一种策略是先产生图像标签的原始猜测，然后使用相邻像素之间的交互来修正该原始猜测。
重复这个修正步骤数次对应于在每一步使用相同的卷积，该卷积在深层网络的最后几层之间共享权重\citep{Jain-et-al-2007}。
这使得在层之间共享参数的连续的卷积层所执行的一系列运算，形成了一种特殊的\gls{RNN}~\citep{Pinheiro-Collobert-2014,Pinheiro-Collobert-2015}。
...
一旦对每个像素都进行了预测，我们就可以使用各种方法来进一步处理这些预测，以便获得图像在区域上的分割\citep{Briggman-et-al-2009,Turaga-et-al-2010,Farabet-et-al-2013}。
一般的想法是假设大片相连的像素倾向于对应着相同的标签。
图模型可以描述相邻像素间的概率关系。
或者，\gls{convolutional_network}可以被训练来最大化地近似图模型的训练目标\citep{Ning-et-al-2005,Thompson-et-al-2014}。
...
\gls{convolutional_network}用于视频的例子，可以参考~\cite{Chen-et-al-2010}。
...
最后，可以使用无监督的标准来学习核。
例如，\cite{Coates-et-al-2011}将$k$均值聚类算法应用于小图像块，然后使用每个学得的中心作为卷积核。
...
随机过滤器经常在\gls{convolutional_network}中表现得出乎意料得好~\cite{Jarrett-et-al-2009,Saxe-et-al-2011,Pinto-et-al-2011,Cox-Pinto-2011}。
\cite{Saxe-et-al-2011}~说明，由卷积和随后的\gls{pooling}组成的层，当赋予随机权重时，自然地变得具有频率选择性和平移不变性。
...
卷积模型的\gls{greedy_layer_wise_pretraining}的经典模型是卷积\gls{DBN}\citep{Lee-et-al-2009}。
\gls{convolutional_network}为我们提供了相对于多层感知机更进一步采用预训练策略的机会。
并非一次训练整个卷积层，我们可以训练一小块模型，就像~\cite{Coates-et-al-2011}使用$k$均值做的那样。
然后，我们可以用来自这个小块模型的参数来定义卷积层的核。
这意味着使用无监督学习来训练\gls{convolutional_network}\emph{并且在训练的过程中完全不使用卷积}是可能的。
使用这种方法，我们可以训练非常大的模型，并且只在推断期间产生高计算成本\citep{Ranzato-et-al-2007b,Jarrett-et-al-2009,Kavukcuoglu-et-al-2010,Coates-et-al-2013}。
...
为了确定关于哺乳动物视觉系统如何工作的许多最基本的事实，神经生理学家David Hubel和Torsten Wiesel合作多年\citep{Hubel-Wiesel-1959,Hubel-Wiesel-1962,Hubel-Wiesel-1968}。
他们的成就最终获得了诺贝尔奖。
...
\begin{enumerate}
  ...
  这些不变性激发了\gls{convolutional_network}中的一些跨通道\gls{pooling}策略，例如maxout单元\citep{Goodfellow-et-al-2013a}。
\end{enumerate}
...
这些祖母细胞已经被证明确实存在于人脑中，在一个被称为\emph{内侧颞叶}的区域\citep{Quiroga-et-al-2005}。
...
\gls{convolutional_network}可以预测IT放电率，并且在执行对象识别任务时与人类（时间有限的情况）非常类似\citep{DiCarlo-2013}。
...
\begin{itemize}
  ...
  研究者已经研发了几种具有视觉机制的视觉模型，但到目前为止还没有成为主导方法\citep{Larochelle-Hinton-2010,Denil-et-al-2012}。
  ...
  最近的V1模型涉及对每个神经元的多个二次过滤器\citep{Rust-et-al-2005}。
...
具有跨多个空间位置的\gls{parameter_sharing}的模型结构，可以追溯到早期关于视觉的联结主义模型\citep{Marr-Poggio-1976}，但是这些模型没有使用现代的反向传播算法和梯度下降。
例如，\citep{Fukushima-1980}结合了现代\gls{convolutional_network}的大多数模型结构设计元素，但依赖于层次化的无监督聚类算法。

\cite{Lang-Hinton-1988}引入反向传播来训练\firstall{TDNNs}。
使用当代术语来说，TDNN是用于时间序列的一维\gls{convolutional_network}。
用于这些模型的反向传播不受任何神经科学观察的启发，并且被一些人认为是生物不可信的。
在基于使用反向传播训练的TDNN成功之后，\cite{LeCun-et-al-1989}通过将相同的训练算法应用于图像的2维卷积来发展现代\gls{convolutional_network}。
...
这种方法被称为\firstgls{reverse_correlation}\citep{Ringach-Shapley-2004}。
...
读者可以参考\citep{Hyvarinen-et-al-2009}来获得自然图像统计领域的综述。
% fig 9.19
\begin{figure}
\ifOpenSource
...
例如，在20世纪90年代，AT\&T的神经网络研究小组开发了一个用于读取支票的\gls{convolutional_network}\citep{LeCun-et-al-1998b}。
到90年代末，NEC部署的这个系统已经被用于读取美国10％以上的支票。
后来，微软部署了若干个基于\gls{convolutional_network}的OCR和手写识别系统\citep{Simard-et-al-2003}。 
关于\gls{convolutional_network}的这种应用和更现代应用的更多细节，参考\chapref{chap:applications}。
读者可以参考\citep{LeCun-et-al-2010}了解2010年之前的更为深入的\gls{convolutional_network}历史。

\gls{convolutional_network}也被用作在许多比赛中的取胜手段。
当前对深度学习的商业兴趣的热度始于\cite{Krizhevsky-et-al-2012}赢得了ImageNet对象识别挑战，但是在那之前，\gls{convolutional_network}也已经被用于赢得前些年影响较小的其他机器学习和计算机视觉竞赛了。
