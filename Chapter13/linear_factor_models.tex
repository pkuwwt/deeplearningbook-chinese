...
这些模型有时被用来作为混合模型的组成模块~\citep{Hinton-et-al-1995a,Ghahramani-Hinton-1996,Roweis-et-al-2002}或者更大的深度概率模型~\citep{Tang-et-al-2012}。
...
在\firstgls{FA}~\citep{Bartholomew-1987,Basilevsky-1994}中，\gls{latent_variable}的先验是一个方差为单位矩阵的\gls{gaussian_distribution}
\begin{align}
\label{eqn:133}
...
之后~\citet{Tipping-Bishop-1999}提出了一种迭代的~\glssymbol{EM}~算法来估计参数$\MW$和$\sigma^2$。
% 481
...
通过\citet{Tipping-Bishop-1999}的研究我们可以发现，当$\sigma \xrightarrow{} 0$时，\gls{PPCA}~退化为~\glssymbol{PCA}。
...
\firstall{ICA}是最古老的\gls{representation_learning}算法之一~\citep{Herault-Ans-1984,Jutten-Herault-1991,Comon-1994,Hyvarinen-1999,Hyvarinen-et-al-2001a,Hinton-et-al-2001,Teh-et-al-2003}。
...
关于非线性~\glssymbol{ICA}~最初的工作可以参考~\citet{Hyvarinen-Pajunen-1999}，它和\gls{ensemble_learning}的成功结合可以参见~\citet{Roberts-Everson-2001,Lappalainen-et-al-2000}。
\glssymbol{ICA}~的另一个非线性扩展是\firstall{NICE}方法~\citep{Dinh-et-al-2014}，这个方法堆叠了一系列可逆变换（在\gls{encoder}阶段），其特性是能高效地计算每个变换的~\gls{jacobian}~行列式。
...
\glssymbol{ICA}~的另一个推广是通过鼓励组内统计依赖关系、抑制组间依赖关系来学习特征组\citep{Hyvarinen-Hoyer-1999,Hyvarinen-et-al-2001b}。
...
\firstall{SFA}是使用来自时间信号的信息学习不变特征的\gls{linear_factor}~\citep{Wiskott-Sejnowski-2002}。
% 484
...
\gls{slow_principle}早于\gls{SFA}，并已被应用于各种模型~\citep{Hinton-1989,Foldiak-1989,Mobahi-et-al-2009,Bergstra-Bengio-2009}。
...
\glssymbol{SFA}~算法~\citep{Wiskott-Sejnowski-2002}先将$f(\Vx;\theta)$定义为线性变换，然后求解如下优化问题
\begin{align}
	\label{eqn:138}
...
当在计算机渲染的3D环境内随机运动的视频上训练时，深度~\glssymbol{SFA}~模型能够学习的特征与大鼠脑中用于导航的神经元学到的特征有许多共同特性~\citep{Franzius-et-al-2007}。
...
深度~\glssymbol{SFA}~也已经被用于学习用在\gls{object_recognition}和姿态估计的特征~\citep{Franzius-et-al-2008}。
...
\citet{Goodfellow-et-al-2013d}描述了不同模型族中的近似推断，如\gls{ss}\gls{sparse_coding}模型，其中先验的样本通常包含许多真正的$0$。
% 488 head
...
\citet{Coates-Ng-2011}证明了在\gls{object_recognition}任务中\gls{sparse_coding}特征比基于参数化的\gls{encoder}（线性-\gls{sigmoid}~\gls{AE}）的特征拥有更好的泛化能力。
受他们的工作启发，\citet{Goodfellow-et-al-2013d}表明一种\gls{sparse_coding}的变体在标签极少（每类20个或更少标签）的情况中比相同情况下的其他\gls{feature_extractor}拥有更好的\gls{generalization}能力。
% 488 
...
允许近似导数的\gls{sparse_coding}模型的修改版本确实存在但未被广泛使用~\citep{Bagnell-Bradley-2009}。
% 488  end
...
此图经~\citet{Goodfellow-et-al-2013d}允许转载。}
...
\gls{linear_factor}，包括~\glssymbol{PCA}~和\gls{FA}，可以理解为学习一个\gls{manifold}~\citep{Hinton-et-al-1997}。
