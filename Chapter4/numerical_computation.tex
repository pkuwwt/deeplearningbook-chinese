...
Theano~\citep{Bergstra-et-al-2010,Bastien-et-al-2012}就是这样软件包的一个例子，它能自动检测并稳定\gls{DL}中许多常见的数值不稳定的表达式。
...
这种技术被称为\firstgls{GD}\citep{Cauchy-1847}。
...
递增带有离散参数的\gls{objective_function}被称为\firstgls{hill_climbing}算法\citep{Russel-Norvig-2003}。
...
使用~\gls{hessian}~矩阵的优化算法被称为\,\textbf{二阶最优化算法}(second-order optimization algorithms)\citep{Nocedal-Wright-2006}，如\gls{newton_method}。
...
有关\gls{convex_optimization}的详细信息，详见~\cite{Boyd-Vandenberghe-2004}或~\cite{Rockafellar-1997}。
...
如果可能的话，在\gls{GD}或\gls{line_search}前将\gls{gradient}投影到\gls{feasible}域的切空间会更高效\citep{Rosen-1960}。
...
这些性质称为\firstacr{KKT}条件\citep{Karush-1939,Kuhn-Tucker-1951}。
...
\end{itemize}
有关~\glssymbol{KKT}~方法的详细信息，请参阅~\cite{Nocedal-Wright-2006}。
